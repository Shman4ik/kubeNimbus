...
- [Logs from resource rows and nested pod lists](docs/engineering/row-logs-and-maximized.md) — One `OpenLogsForAsync` path for L, hover buttons, Events and Argo managed resources; a named pod is re-read so a deleted pod produces a warning. The screenshot harness had to hover a nested row after DataGrid materialization to expose its button.
