# kubeNimbus — Codex working notes
...
- [Logs from resource rows and nested pod lists](docs/engineering/row-logs-and-maximized.md) — One `OpenLogsForAsync` path for L, hover buttons, Events and Argo managed resources; named objects are re-read with pane cancellation, and right clicks select their row before the Logs menu acts. Argo's status lacks selectors, so its capability is checked on demand.
...
| `AGENTS.md` (this file) | Whoever is changing the code. The engineering contract and the *why* behind every rule. |
...
`docs/BACKLOG.md` is the queue, and `.Codex/` holds the machinery that works
...
Moved to [`design/AGENTS.md`](design/AGENTS.md), which loads when working under `design/`. The short version: nothing in `design/*.svg` is hand-edited (the `.af` files are the art), and the base and broom are shared byte-for-byte with pgNimbus, so a change to either is a pair of PRs.
...
# Headless visual check (no display, e.g. Codex Cloud) — see below.
...
containers, Codex Cloud), `dotnet publish src/KubeNimbus.App -c Release
...
For environments with no display and no DevTools MCP (Codex Cloud
...
The release workflow, installers, Microsoft Store (MSIX) identity, the assembly-name coupling and the 0.5 GB Actions storage budget are in the `release` skill ([`.Codex/skills/release/SKILL.md`](.Codex/skills/release/SKILL.md)). Load it before cutting a release or touching `.github/workflows`, `installer/` or the packaging scripts. Two rules that must not wait for it: **every `upload-artifact` sets `retention-days`**, and **the MSIX identity in `installer/msix/Package.appxmanifest` and the MSI `UpgradeCode` are never edited**.
