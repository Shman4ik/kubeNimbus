...
class, its ids were Inkscape's, and the dark twin it then had no longer matched
it path for path.
...
file liftable into a sibling mark (pgNimbus shares the broom):
  ...
  * the two base circles stay <circle>, so the plate (r="512") and the field
    (r="360") can be found and compared against pgNimbus's by value;
...
HEADER = """<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" role="img" aria-labelledby="logo-title">
  ...
  <!-- ============================================================
       ...
       mark without touching the geometry. There is no dark twin:
       the plate carries the mark's own contrast, so one file reads
       on light and dark surfaces alike (the same rule as pgNimbus).
       ============================================================ -->
