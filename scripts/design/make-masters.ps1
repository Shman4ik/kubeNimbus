...
#
# Every size comes off the one vector master, including 16 and 24 - the same
# rule as pgNimbus. kubeNimbus used to carry two simplified marks for those
# sizes (a drawn 24px logo-small and a generated 16px logo-micro, each with a
# dark and a plated colourway) plus a dark twin of the full mark. They were
# removed (2026-09) because the family should read as one mark everywhere, and
# eight extra files that each had to be regenerated or redrawn on a broom edit
# were buying a small-size legibility gain at the cost of a mark that no longer
# looked like itself. If a size ever does stop reading, the fix is still a
# simplified *mark* fed in here, never a hand-painted PNG that nothing can
# regenerate.
#
...
# ------------------------------------------------------------ window masters
# One plated mark for both themes, same file twice - the same choice pgNimbus
# made. These used to be theme-tinted transparent line art (the disc stripped
# out of logo.svg and of a palette-swapped logo-dark.svg), plus separate 16 and
# 24px cuts from the simplified marks. That was two more colourways of a mark
# that everywhere else ships in exactly one, because the plate already carries
# its own contrast. Two identical files rather than one is kept only because
# make-app-icons.ps1 (window-icon-{light,dark}.ico, the MSIX altform-{unplated,
# lightunplated} tiles) still keys off two file names. The MSIX "unplated"
# tiles therefore carry a plate too: Windows adds its own backplate around
# them, so that surface gets a plate inside a plate, accepted deliberately for
# one mark everywhere.
...
# colourway. icon-1024.png is the same render; this exists so that anything
# wanting "the logo" (a site header, a talk slide) finds it under masters/logo/
# rather than reaching into the icon directory for a file that means something
# else.
Export-Png $light (Join-Path $logoDir 'logo.png') 1024

# ------------------------------------------------------------------ wordmark
# Horizontal lockup: the mark at 240px beside "kubeNimbus" set in Segoe UI Bold.
...
# to change is the type: "kubeNimbus" set in ink is unreadable on a dark README.
foreach ($v in @(
...
# card pgNimbus uses, carrying the "kubeNimbus" wordmark and the
# one-line tagline, laid out exactly as pgNimbus's so the two repositories'
# link previews read as one family. GitHub renders the card uncropped at its
# native 2:1, so the lockup plus tagline survives where it is seen the most.
