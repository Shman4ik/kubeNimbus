---
name: "source-command-backlog-cycle"
...
# source-command-backlog-cycle

Use this skill when the user asks to run the migrated source command `backlog-cycle`.
...
constraint from the item's Notes column. Do not paste `AGENTS.md` — it reads that
itself.
...
criteria contradict `AGENTS.md`, or the design is unsound — stop the cycle and
