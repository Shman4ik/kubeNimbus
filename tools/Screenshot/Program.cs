...
void Capture(string name, ThemeVariant theme, Func<Control> build)
{
    ...
    if (name is "ux-workload-pods" or "cluster-tab-node-detail-pods")
    {
        UxInteractionChecks.RightClickNestedPodRow(window);
        UxInteractionChecks.HoverNamedPodRow(window);
    }
