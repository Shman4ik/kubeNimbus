...
/// <summary>
/// The keys a shortcut can use. Deliberately a Core-local enum rather than Avalonia's
/// <c>Key</c>: the catalog has to stay UI-free (CLAUDE.md rule 1) and unit-testable,
/// so the App owns the one mapping from these to real keys.
/// </summary>
...
/// <summary>
/// Modifiers on a <see cref="Chord"/>. <see cref="Command"/> is the abstract primary
/// modifier that renders as Ctrl or Cmd depending on the resolved scheme;
/// <see cref="Control"/> is a literal Ctrl that stays Ctrl even under the Cmd scheme.
///
/// <para>
/// The literal variant is not hypothetical here: the exec pane's interrupt is Ctrl+C
/// and its end-of-input is Ctrl+D on every platform, because those are terminal
/// control characters rather than app shortcuts — and on macOS Cmd+C is Copy, which
/// is exactly what the exec pane leaves it as.
/// </para>
/// </summary>
[Flags]
...
/// <summary>
/// One key combination, stored abstractly so the same descriptor renders as Ctrl+K or
/// Cmd+K without the catalog knowing which platform it is on.
/// </summary>
public readonly record struct Chord(CommandKey Key, ChordModifiers Modifiers = ChordModifiers.None)
{
    /// <summary>
    /// The individual key caps, in display order (Cmd, Ctrl, Alt, Shift, key) — what
