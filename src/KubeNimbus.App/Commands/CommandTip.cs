...
/// <summary>
/// Renders a control's tooltip as "what it does (its shortcut)", with the shortcut read
/// from <see cref="CommandCatalog"/> instead of typed into the markup. Same contract as
/// the key bindings and the F1 sheet: a gesture is stated once in Core and every surface
/// is a projection of it — a tooltip that hardcodes "(F1)" goes stale the moment the
/// chord moves or the user picks the Cmd scheme.
///
/// <code>
///   &lt;Button cmd:CommandTip.Text="Show or hide the sidebar" cmd:CommandTip.Command="ToggleSidebar" /&gt;
/// </code>
///
/// <c>Text</c> alone is a plain tooltip; <c>Command</c> alone falls back to the
/// catalog's own title. The label re-renders live when the Ctrl/Cmd preference changes,
/// so an open window never shows the other platform's chord.
/// </summary>
public static class CommandTip
{
    /// <summary>The human sentence — what the control does, with no gesture in it.</summary>
    public static readonly AttachedProperty<string?> TextProperty =
        AvaloniaProperty.RegisterAttached<Control, string?>("Text", typeof(CommandTip));

    /// <summary>
    /// The catalog entry whose chord is appended in parentheses. Nullable on purpose:
    /// the enum's zero value is a real command, so a non-nullable property would treat
    /// setting it as "no change" and never raise the handler that composes the tip.
    /// </summary>
    ...
    // Guards the one-time Hotkeys.Changed wiring: both properties are usually set on
    // the same control, and each set runs through Apply.
    ...
    private static string? Compose(Control control)
    {
        ...
        return descriptor.ShortcutLabel(Hotkeys.PrimaryLabel) is { Length: > 0 } shortcut
            ? $"{text} ({shortcut})"
            : text;
    }
