...
- [Logs from resource rows and nested pod lists](docs/engineering/row-logs-and-maximized.md) — The row's logs icon, Shift+L, the maximized preference, and one `OpenLogsForAsync` path from workload, node, Event and Argo rows. Named pods are re-read before opening because node and Argo lists can be stale; the screenshot harness must hover a nested row after DataGrid materialization to expose its button.
